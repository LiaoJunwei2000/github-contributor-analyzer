...
if run_btn:
    ...
    with st.status("正在分析中...", expanded=True) as status:
        ...
        # 续传：读取已有完整 Profile，跳过这些用户的 API 请求
        skip_logins = set()
        existing_profiles = {}
        if resume_mode:
            existing_profiles = get_complete_profiles(repo)
            skip_logins = set(existing_profiles.keys())
            st.write(f"⚡ 续传模式：已有 **{len(skip_logins)}** 位用户数据，"
                     f"仅抓取剩余 **{len(merged) - len(skip_logins)}** 位...")
        else:
            st.write(f"🔍 并发抓取 {len(merged)} 位用户的 Profile 信息...")

        enriched = enrich_with_user_details(merged, token, skip_logins=skip_logins)

        # 将已跳过用户的 Profile 字段从 DB 记录回填
        _profile_fields = [
        ...
        # 断点检测：total_changes 和 followers 均为 0/None
        incomplete = [
            r for r in enriched
            if not (r.get("total_changes") or 0) and r.get("followers") is None
        ]
        if incomplete:
            status.update(label="分析完成（数据不完整）⚠️", state="complete", expanded=False)
        else:
    ...
    if incomplete:
        st.warning(
            f"⚠️ **检测到断点：{len(incomplete)} 位用户数据不完整**（total_changes 与 followers 均为空）。\n\n"
            f"可能原因：抓取过程中触发了 GitHub API Rate Limit。\n\n"
            f"**解决方法**：勾选上方 **「⚡ 续传」** 后重新点击「🚀 开始分析」，"
            f"将自动跳过已完成的用户，仅补抓缺失数据。",
        )
