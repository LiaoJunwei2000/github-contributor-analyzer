...
- `total_additions` — 累计新增行数
- `total_deletions` — 累计删除行数
- `net_lines` — 净增行数
- `contributions_on_default_branch` — 默认分支贡献数
    """)
...
if run_btn:
    ...
    with st.status("正在分析中...", expanded=True) as status:
        ...
        if not all_contribs:
            ...
            st.error("未能获取贡献者数据。")
            st.stop()
        st.write(f"✅ 获取到 **{len(all_contribs)}** 位贡献者")
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "account_created": st.column_config.TextColumn("账号创建时间"),
        },
    )
    ...
    st.caption(f"共 {len(enriched)} 位贡献者，{len(CSV_FIELDS)} 个字段")
